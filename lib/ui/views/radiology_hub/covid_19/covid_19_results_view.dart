...
class CovidResultsView extends StatelessWidget {
  const CovidResultsView({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final vm = context.watch<CovViewModel>();
    return BaseViewWidget(
      ...
      body: Column(
        children: <Widget>[
          ...
          Text(
            'Covid-19: ${vm.modelResults?.covid?.toStringAsFixed(3)}',
            textAlign: TextAlign.center,
            style: const TextStyle(fontSize: 14),
          ),
          Text(
            'Non-COVID Lungs Infection: ${vm.modelResults?.lungsInfection?.toStringAsFixed(3)}',
            textAlign: TextAlign.center,
            style: const TextStyle(fontSize: 14),
          ),
          Text(
            'Viral Pneumonia: ${vm.modelResults?.pneumonia?.toStringAsFixed(3)}',
            textAlign: TextAlign.center,
            style: const TextStyle(fontSize: 14),
          ),
          SizedBox(
