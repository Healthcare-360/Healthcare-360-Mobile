...
class CervicalResultsView extends StatelessWidget {
  const CervicalResultsView({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final vm = context.watch<CervicalViewModel>();
    return BaseViewWidget(
      ...
      body: Column(
        children: <Widget>[
          ...
          Text(
            // ignore: unnecessary_null_comparison
            'Stage 1 (1A, 1B & 2A): ${vm.cervicalCancerResults?.stage1?.toStringAsFixed(3)}',
            textAlign: TextAlign.center,

            style: const TextStyle(fontSize: 14),
          ),
          Text(
            'Stage 2 (2B, 3 & 4A): ${vm.cervicalCancerResults?.stage2?.toStringAsFixed(3)}',
            textAlign: TextAlign.center,
            style: const TextStyle(fontSize: 14),
          ),
          Text(
            'Stage 3 (4B): ${vm.cervicalCancerResults?.stage3?.toStringAsFixed(3)}',
            textAlign: TextAlign.center,
            style: const TextStyle(fontSize: 14),
          ),
          SizedBox(
